...
class BPF {
  ...
  constructor(nop, inc, pnc, mnc){
      this.m = nop; //numberOfParticles
      this.p0 = inc; //initialNoiseCovariance
      this.q = pnc; //processNoiseCovariance
      this.r = mnc; //measurementNoiseCovariance

      var math = require('mathjs');
      this.theLikelihoodfunction = (1/(2*math.pi*r)^(1/2)*math.exp(-(math.forEach(x, ^2))))
  }

  size(x, dim){
    var size = math.size(x);
    return math.subset(size, math.index(dim-1));
  }



  bPF_xhat(f,h,pe,q,p0,m,y){
    n = math.size()
  }
}
**/
